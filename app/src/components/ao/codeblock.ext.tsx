...
export default function CodeBlockExt() {
    ...
    return <div className="betteridea-codeblock min-h-[130px] w-full bg-transparent">
        {aosProcess ?
            <CodeCell aosProcess={aosProcess} /> : <>
                {!walletAddr && <Button className="m-1" variant="outline" onClick={async () => {
                    try {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                }}>{walletAddr || "connect wallet"}</Button>}
                {
                    walletAddr && <Button className="m-1" variant="outline" disabled={spawning} onClick={async () => {
                        ...
                        setSpawning(false);
                    }}>{spawning ? "spawning..." : "Spawn Process"}</Button>
                }
            </>
        }
    </div>
