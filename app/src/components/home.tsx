...
export default function Home({ contracts, setActiveMenuItem, setActiveContract, setActiveFile }: { contracts: ContractsType, setActiveMenuItem: (val: string) => void, setActiveContract: (val: string) => void, setActiveFile: (val: string) => void }) {
    const [recents, _] = useState<string[]>(JSON.parse(localStorage.getItem("recents")) || [])

    function ContractCard({ name, src, state }: { name: string, src?: string, state?: string }) {
        return <div className="ring-1 rounded ring-white/50 p-2 px-3 flex flex-col gap-2 hover:bg-white/5 min-w-[150px]">
    ...
    return <div className=" h-full flex flex-col gap-1 items-center justify-center">
        ...
        <div className="flex flex-col gap-5 justify-start items-start w-full px-10">
            <div>
                <div className="text-xl my-1">{recents.length == 0 ? "No recently opened contracts" : "Recently opened contracts"}</div>
