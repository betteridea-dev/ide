...
import { setActiveContract, setActiveFile, setActiveSideNavItem } from "@/store/app-store";
...
export default function Home({ contracts }: { contracts: ContractsType }) {
  ...
  const [recents] = useState<string[]>(typeof window != "undefined" ? JSON.parse(localStorage.getItem("recents")!) || [] : []);
  ...
  function ContractCard({ name, src, state }: { name: string; src?: string; state?: string }) {
    return (
      <Card className="flex flex-col min-w-[160px] gap-3 p-3" suppressHydrationWarning>
    ...
    <div className="flex h-full flex-col items-center justify-center gap-2">
      ...
      <div className="flex w-full flex-col items-start justify-start gap-5 px-10">
        <div suppressHydrationWarning>
          <div className="my-3 text-xl" suppressHydrationWarning>
            {recents.length == 0 ? "No recently opened contracts" : "Recently opened contracts"}
          </div>

          <div className="flex gap-3" suppressHydrationWarning>
            {recents.map((recent, _) => {
              return <ContractCard name={recent} key={_} />;
            })}
          </div>
        </div>
