...
export default function WarpTest({ contracts, target }: { contracts: ContractsType; target: string }) {
    ...
    <div className="flex h-full flex-col items-center justify-center gap-5">
      ...
      <div className="grid w-full grid-cols-2 gap-5 p-5">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-row gap-4">
              ...
              <RadioGroup value={callType} name="calltype" onValueChange={(val) => setCallType(val as "read" | "write")} className="flex flex-row">
          ...
          <div className="h-full w-full overflow-clip rounded p-0.5 ring-1 ring-white/20">
            <iframe className="h-full w-full rounded" src={`/?editor&language=json&file=input/state.json`} />
          </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <p>Result</p>
          <pre className={`overflow-scroll rounded bg-white/10 p-1 ${success ? "text-green-400" : "text-red-400"}`}>{result || "..."}</pre>

          <div className="h-2"></div>

          <p>Latest State</p>
          <pre className="overflow-scroll rounded bg-white/10 p-1">{latestState || "..."}</pre>
        </div>
      </div>
    </div>
  );
