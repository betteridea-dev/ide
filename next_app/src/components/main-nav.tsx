...
import { setAppMode, setActiveSideNavItem, setActiveFile } from "@/store/app-store";
...
*/
export default function MainNavBar({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="flex h-20 min-h-[5rem] px-6 bg-[#111111]">
      <div className="flex justify-center items-center gap-2">
        ...
        <h1 className="bg-gradient-to-r from-[#006F86] to-white bg-clip-text text-2xl font-bold tracking-tight text-transparent">BetterIDEa</h1>
      </div>
...
export function MainNavFileTab({ filename }: { filename: string }) {
  ...
  return (
    <div
      className={cn("h-fit w-fit p-1 px-2 cursor-pointer items-center justify-center flex border rounded-lg border-white/10", activeFile == filename && "bg-white/10")}
      onClick={() => {
