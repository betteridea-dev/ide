...
export default function CodeCell() {
    ...
    useEffect(() => {
        if (localAosProcess[appname]) {
            ...
            window.processId = localAosProcess[appname];
            setAosProcess(localAosProcess[appname]);
            console.log("setting aos process: ", localAosProcess[appname]);
            setIsSpawning(false);
            if (window.parent) {
  ...
  transactions(
    ...
    async function runCellCode() {
        ...
        window.processId = localProcesses[appname];
        setAosProcess(localProcesses[appname]);
        const r = await runLua(code_, window.processId, [
            { name: "External-App-Name", value: appname },
            { name: "External-Url", value: `${btoa(url)}` },
            { name: "File-Type", value: "External-Code-Cell" },
        ]);
